...
class _MovementLogsScreenState extends State<MovementLogsScreen> {
  ...
  Future<void> _generateTestLogs() async {
    try {
      final loggingService = ActivityLoggingService();
      ...
      await loggingService.logScreenAccess('Inventory List Screen');
      await Future.delayed(const Duration(milliseconds: 100));
      
      await loggingService.logItemAdded(
        itemId: 'TEST001',
        itemName: 'Test Bolt M10',
        itemData: {
          'sapCode': 'SAP-12345',
          'currentStock': 50,
          'rackNumber': 'A-01',
      ...
      await loggingService.logItemEdited(
        itemId: 'TEST002',
        itemName: 'Test Nut M8',
        changes: {
          'quantity': {'old': 30, 'new': 45},
          'location': {'old': 'B-02', 'new': 'C-05'},
        },
      );
      await Future.delayed(const Duration(milliseconds: 100));
      
      await loggingService.logReceiveItem(
        itemId: 'TEST003',
        itemName: 'Test Washer 12mm',
        quantity: 100,
        source: 'Supplier ABC',
      ...
      await loggingService.logIssueItem(
        itemId: 'TEST004',
        itemName: 'Test Screw M6',
        quantity: 25,
      ...
      await loggingService.logUserRoleChange(
        targetUserId: 'user123',
        targetUserEmail: 'technician@example.com',
        oldRole: 'Technician',
        newRole: 'Storekeeper',
      );
      await Future.delayed(const Duration(milliseconds: 100));
      
      await loggingService.logScreenAccess('User Management Screen');
      await Future.delayed(const Duration(milliseconds: 100));
      
      await loggingService.logItemDeleted(
        itemId: 'TEST005',
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Error generating test logs: $e'),
    ...
    try {
      ...
      final confirm = await showDialog<bool>(
        ...
        builder: (context) => AlertDialog(
          title: const Text('Migrate Existing Data'),
          content: const Text(
            'This will import all existing receivings and issuance transactions into Activity Logs.\n\nThis operation may take a few moments.',
          ),
          actions: [
            TextButton(
      ...
      // Migrate receivings data
      final receivingsSnapshot = await FirebaseFirestore.instance
          .collection('receivings')
          .get();

      for (final doc in receivingsSnapshot.docs) {
        final data = doc.data();
        ...
        final existingLog = await FirebaseFirestore.instance
            .collection('activity_logs')
            .where('itemId', isEqualTo: data['sapCode'])
            .where('action', isEqualTo: 'RECEIVE_ITEM')
        ...
        if (existingLog.docs.isEmpty) {
          await FirebaseFirestore.instance.collection('activity_logs').add({
            'userId': 'migrated',
            'userEmail': data['supplier'] ?? 'Unknown Supplier',
            'userRole': 'Admin',
            'action': 'RECEIVE_ITEM',
      ...
      // Migrate issuance data
      final issuanceSnapshot = await FirebaseFirestore.instance
          .collection('issuance')
          .get();

      for (final doc in issuanceSnapshot.docs) {
        final data = doc.data();
        final timestamp = (data['timestamp'] as Timestamp?)?.toDate();
        ...
        final existingLog = await FirebaseFirestore.instance
            .collection('activity_logs')
            .where('itemId', isEqualTo: data['sapCode'])
            .where('action', isEqualTo: 'ISSUE_ITEM')
            .where('timestamp', isEqualTo: timestamp)
            .limit(1)
            .get();
        ...
        if (existingLog.docs.isEmpty) {
          await FirebaseFirestore.instance.collection('activity_logs').add({
            'userId': 'migrated',
            'userEmail': data['technicianName'] ?? 'Unknown Technician',
            'userRole': 'Admin',
            'action': 'ISSUE_ITEM',
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('✓ Migrated $migratedCount transactions to Activity Logs'),
            backgroundColor: Colors.green,
